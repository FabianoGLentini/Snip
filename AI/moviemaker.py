...
start = 1;
# Ensure exactly 7 valid concepts
if len(concepts) == 7 and all(len(concept.split()) >= 10 for concept in concepts):
    for concept in concepts:
        print(f"Processing: {start}")
        generate_video(concept, start)
        start+=1
else:
    print("Error: The model did not return exactly 7 full-sentence concepts.")
